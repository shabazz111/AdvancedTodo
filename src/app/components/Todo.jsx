...
const Todo = ({ todo, onChangeCrossHandler,onDeleteHandler }) => {
  ...
  const [checked, setChecked] = React.useState(isCross);

  return (
    <div className="w-full items-center my-5 border flex shadow-lg bg-gray-300 justify-between p-3">
      <span
        className={
          checked
            ? "text-with-line text-gray-500 truncate w-5/6"
            : "truncate w-5/6"
        }
      >
        {title}
